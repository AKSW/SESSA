...
public class QuerySearch {
	...
	private void SearchInLemonCluster(IndexerInterface node, String label, String index) {
		...
		for (SearchHit hit : results) {
			...
			Double EnergyScore = EnergyCalc.energy_score((String) result.get("label"), label);
	...
	public void DatatypeNormalize(IndexerInterface node, String label) {
		...
		for (SearchHit hit : results) {
			Map<String, Object> result = hit.getSource();
			Number = Number * Double.parseDouble(result.values()
					.toString()
					.split(", ")[0].replaceAll("\\[", ""));
			String LabelFactUnit = result.values()
					.toString()
					.split(", ")[2];
			String StdUnit = result.values()
					.toString()
					.split(", ")[3].split("-")[0];

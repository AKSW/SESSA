package org.dbpedia.keywordsearch.stanford;
...
public class StanfordRDFizer {
	...
	public String disambiguate(final String label) {
		...
		System.out.println(preAnnotatedText);
	...
	public static void main(final String[] args) throws IOException {
		...
		Dataset[] datasets = { Dataset.QALD6_Test_Hybrid };
		for (Dataset dataset : datasets) {
			List<IQuestion> questions = LoaderController.load(Dataset.QALD6_Test_Hybrid);
			...
			for (IQuestion q : questions) {
				...
				for (String answer : q.getGoldenAnswers()) {
					...
					System.out.println(stan.disambiguate(answer));

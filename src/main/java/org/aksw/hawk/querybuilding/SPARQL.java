	...
	public SPARQL() {
		try {
			...
			//qef = FluentQueryExecutionFactory.http("http://dbpedia.org/sparql").config().withCache(cacheFrontend).end().create();
			 qef = FluentQueryExecutionFactory.http("http://dbpedia.org/sparql", Lists.newArrayList("http://dbpedia.org")).config()
			...
			// QueryExecutionFactoryHttp("http://localhost:3030/ds/sparql");
			// qef = new
			// qef = QueryExecutionFactoryHttp("http://dbpedia.org/sparql","http://dbpedia.org");
